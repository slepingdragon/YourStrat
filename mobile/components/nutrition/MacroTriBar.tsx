...
const ROW_DEFS: { key: RowKey; metricId: TodayMetricId; labelKey: string; color: string }[] = [
  { key: "protein", metricId: "protein", labelKey: "metric.protein", color: colors.protein },
  { key: "carbs", metricId: "carbs", labelKey: "metric.carbs", color: colors.carbs },
  { key: "fat", metricId: "fat", labelKey: "metric.fat", color: colors.fat },
  { key: "sugar", metricId: "sugar", labelKey: "metric.sugar", color: colors.warning },
];
...
function Row({
  ...
  const valueText = t("nutrition.macroValueG", { value: Math.round(value), target: Math.round(target) });
...
function MacroTriBarImpl({ totals, targets, onMacroPress }: Props) {
  ...
  return (
    ...
    >
      {ROW_DEFS.map(({ key, metricId, labelKey, color }) => {
        const value =
          key === "protein"
            ? totals.protein_g
            : key === "carbs"
        ...
        return (
          <Row
            ...
            accessibilityLabel={t("nutrition.macroRowA11y", { label, value: Math.round(value), target: Math.round(target) })}
          />
        );
      })}
    </View>
